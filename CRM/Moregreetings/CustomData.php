...
class CRM_Moregreetings_CustomData {
  ...
  public function syncOptionGroup($source_file) {
    ...
    elseif ($optionGroup === 'FAILED') {
      ...
      $this->log(
        self::CUSTOM_DATA_HELPER_LOG_ERROR,
        "Couldn't create/update OptionGroup: " . json_encode($data)
      );
      return;
    }
    else {
  ...
  public function syncCustomGroup($source_file): void {
    $force_update = FALSE;
    $data = json_decode(file_get_contents($source_file), TRUE);
    if (empty($data)) {
    ...
    elseif ($customGroup === 'FAILED') {
      ...
      $this->log(
        self::CUSTOM_DATA_HELPER_LOG_ERROR,
        "Couldn't create/update CustomGroup: " . json_encode($data)
      );
      return;
    }
    else {
  ...
  protected function updateEntity($entity_type, $requested_data, $current_data, $required_fields = [], $force = FALSE) {
    ...
    if ($force || !empty($update_query)) {
      ...
      $this->log(
        self::CUSTOM_DATA_HELPER_LOG_INFO,
